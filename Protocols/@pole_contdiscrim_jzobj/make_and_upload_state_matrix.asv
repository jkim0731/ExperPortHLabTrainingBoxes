...
%    slid = 2^2; % Signal line for signaling trial numbers and fiducial marks.
   ...
   RealTimeStates.poles_descend_and_sample = b+2;
   RealTimeStates.wait_for_answerlick = b+3;
   RealTimeStates.reward = b+4;
   RealTimeStates.extra_iti = b+6;
   RealTimeStates.miss = b+7;
   RealTimeStates.correct_nogo = b+8;
   RealTimeStates.poles_ascend = b+9;
   RealTimeStates.airpuff = b+10;                             
   ...
   switch SessionType % determined by SessionTypeSection
      ...
      case 'Water-Valve-Calibration'
       ...
       case 'Discrim_DHO' % Task designed to replicate Dan O'Connor's task for his physiology paper
           pfdr = 0.001; % Duration of airpuff.
           eiti = max(.001,ExtraITIOnError); % Defined in TimesSection.m.
           sptm = SamplingPeriodTime;  % Defined in TimesSection.m.
           ...
           if next_side=='r' % 'r' means a go trial.
               onlk2 = RealTimeStates.reward(1); % 4
               tmout = RealTimeStates.miss(1); % 8
           else
               onlk2 = RealTimeStates.airpuff(1); % 10
               tmout = RealTimeStates.correct_nogo(1); % 7
           end
           
           stm = [stm ;
               %Cin Cout    Tup   Tim       Dou         Aou  (Dou is bitmask format)
               b     b      101   .01       slid        0  ; ... % trigger camera and EPHUS; 10 ms later jump to state 101 to give trial num bit code.% 40 sBC: bitcode. slid will evoke TTL0 signal to scanbox. concurrent with durid evoking up event of TTL1 (resulting in event type 3 in scanbox)
               b+1   b+1    b+2   prepause  0           0 ; ...
               b+2   b+2    b+3   sptm      pvid        0  ; ... % wait for poles to descend, wait duration of sampling period.
               onlk2 onlk2  tmout aptm      pvid        0  ; ... % wait for lick for aptm seconds.
               b+4   b+4    b+5   wvtm      pvid+wvLid   0  ; ... % reward tone + water - HIT
               b+5   b+5    b+9   2-wvtm    pvid        0  ; ... % Drinking time.
               b+10  b+6    b+9   eiti      pvid        0 ; ... % incorrect lick, extra ITI - FALSE ALARM
               b+7   b+7    b+9   .001      pvid        0  ; ... % didn't lick before timeout - MISS
               b+8   b+8    b+9   .001      pvid        0  ; ... % correct nogo.
               b+9   b+9    35    postpause 0           0  ; ... % raise poles by unsetting pvid, then go state 35 for new trial
               b+10  b+10   b+6   pfdr      pvid        0 ; ... % FALSE ALARM. Second extra ITI state, to retrigger extra ITI.
               ];
...
%            arrival indication 02/22/16 JK for scanbox           
           ...
           x = double(dec2binvec(trialnum)');% insert 0 before start, 
...
%            pole down. First gap time is 10 ms (sBC TimeupSt).
       ...
       case 'Detection_SP' % Task more tailored to imaging with, e.g., pre-pauses for F_0 collection
           ...
           stm = [stm ;
               ...
               b        b      101      .01       slid  0 ; ...% 40 sBC: bitcode. slid will evoke TTL0 signal to scanbox. concurrent with durid evoking up event of TTL1 (resulting in event type 3 in scanbox)
...
%            pole down. First gap time is 10 ms (sBC TimeupSt).
    ...
    if strcmp(SessionType,'Discrim_DHO')
        stm(b:end,5) = stm(b:end,5) + durid; % pad all Dout with durid, except for state 35 and 0. sBC = 40; Indicating duration of whisker video imaging.
    end
